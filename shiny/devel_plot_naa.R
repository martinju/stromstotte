...
setnames(dc1,"median","stotte_faktisk")
setnames(dc2,c("median","quantile_0.025","quantile_0.975","lower_bound"),c("stotte_estimat","stotte_lower_CI","stotte_upper_CI","stotte_lower_bound"))
...
plot_strompris_naa_dt[,totalpris_faktisk:=spotpris+nettleie-stotte_faktisk]
plot_strompris_naa_dt[,totalpris_estimat:=spotpris+nettleie-stotte_estimat]
...
plot_strompris_naa_dt0 <- plot_strompris_naa_dt[,.(datetime,spotpris,nettleie,totalpris_faktisk,totalpris_estimat,totalpris_lower_CI,totalpris_upper_CI)]
...
plot_strompris_naa_dt_ints <- plot_strompris_naa_dt00[,.(datetime,totalpris_lower_CI,totalpris_upper_CI)]
plot_strompris_naa_dt_ints[,type:="totalpris_estimat"]

plot_strompris_naa_dt_melted <- melt(plot_strompris_naa_dt00[,.(datetime,spotpris,nettleie,totalpris_faktisk,totalpris_estimat)],
...
p_history <- ggplot(data=plot_dt_final,mapping=aes(x=datetime,y=pris,col=type,fill=type))+
  geom_line()+
  geom_ribbon(aes(ymin = totalpris_lower_CI, ymax = totalpris_upper_CI), alpha = 0.5)+
